...
/// <summary>
        ...
        /// Descripción: Descripción detallada del metodo, procure especificar todo el metodo aqui.
        /// </remarks>
        protected void MostrarMensaje(string mensaje, TipoMensaje tipoMensaje)
        {
            if (!string.IsNullOrEmpty(mensaje))
            {
                mensaje = mensaje.Replace("'", string.Empty);

                short identificadorTipoMensaje = 0;
                string estiloMensaje = string.Empty;

                switch (tipoMensaje)
                {
                    case TipoMensaje.Ok:
                        estiloMensaje = Resources.GlobalWeb.Estilo_MensajeOK;
                        break;

                    case TipoMensaje.Error:
                        estiloMensaje = Resources.GlobalWeb.Estilo_MensajeERROR;
                        identificadorTipoMensaje = 1;
                        break;
                }
                ...
                if (dialogo != null)
                {
                    dialogo.MostrarMensaje(this.Page, mensaje, estiloMensaje, identificadorTipoMensaje);
                }
            }
        }
