    ...
    public partial class AdministradorFacActividad : WebPage
    {
        ...
        protected void BtnCerrarCorte_Click(object sender, EventArgs e)
        {
            ...
            if (listaCorte.Count() > 0)
            {
                ...
                CargaObjetos.OrdenamientoGrilla(grillaVentas, ventasFacturar);
            }
            else
            {
                grillaVentas.DataSource = Ventas;
        ...
        /// Descripción: Descripción detallada del metodo, procure especificar todo el metodo aqui.
        /// </remarks>
        protected void grillaEntidades_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            int indiceFila = Convert.ToInt32(e.CommandArgument);
            var identificadorEntidad = (Label)grillaEntidades.Rows[indiceFila].FindControl("lblIdEntidad");
            ...
            switch (e.CommandName)
            {
                case CliCountry.SAHI.Comun.Utilidades.Global.SELECCIONAR:
                    ...
                    ActivarCeldaGridView(grillaEntidades, indiceFila);
                    break;
            }
        }
        ...
        protected void ImgActualizarVinculaciones_Click(object sender, ImageClickEventArgs e)
        {
            ...
            foreach (GridViewRow registro in grillaEntidades.Rows)
            {
        ...
        protected void ImgbtnVincularEntidad_Click(object sender, ImageClickEventArgs e)
        {
            ...
            ucVincularEntidad.txtOrden.Text = grillaEntidades.Rows.Count > 0 ? Convert.ToString(grillaEntidades.Rows.Count + 1) : Convert.ToString(1);
        ...
        protected void ImgCortesFacturacion_Click(object sender, ImageClickEventArgs e)
        {
            ...
            var gridVentas = grillaVentas.Rows.Cast<GridViewRow>();
        ...
        private void ActivarCeldaGridView(GridView grillaVinculaciones, int indiceFila)
        {
            foreach (GridViewRow fila in grillaVinculaciones.Rows)
            {
                if (fila.FindControl("imbSelect") != null)
                {
                    ...
                    {
                        ImageButton botonselect = (ImageButton)fila.FindControl("imbSelect");
                        botonselect.ImageUrl = "~/App_Themes/SAHI/images/pagSiguiente.png";
                    }
                    else
                    {
                        ImageButton botonselect = (ImageButton)fila.FindControl("imbSelect");
                        botonselect.ImageUrl = "~/App_Themes/SAHI/images/seleccionar.png";
                    }
                }
            }
        }
        ...
        /// <returns></returns>
        private Vinculacion BuscarVinculacionSeleccionada(int identificadorEntidad, int ordenVinculacion)
        {
            var vinculacionSeleccionda = from
            ...
            return vinculacionSeleccionda.FirstOrDefault();
        }
        ...
        private void CargarVentas(int identificadorAtencion, List<int> ventasMarcadas)
        {
            ...
            if (respuesta.Ejecuto)
            {
                ...
                if (Ventas != null)
                {
                    ...
                    if (consultar)
                    {
                        CargaObjetos.OrdenamientoGrilla(this.Page, grillaVentas, Ventas);
        ...
        private void CargarVinculaciones(Vinculacion atencion)
        {
            ...
            if (resultado.Ejecuto)
            {
                ...
                {
                    CargaObjetos.OrdenamientoGrilla(grillaEntidades, VinculacionesDeAtencion);

                    if (VinculacionesDeAtencion.Count > 0)
                    {
                        grillaEntidades_RowCommand(this, new GridViewCommandEventArgs(grillaEntidades.Rows[0], new CommandEventArgs(CliCountry.SAHI.Comun.Utilidades.Global.SELECCIONAR, 0)));
                        grillaEntidades.SelectedIndex = 0;
                    }
                }
            }
            else
        ...
        /// Descripción: Descripción detallada del metodo, procure especificar todo el metodo aqui.
        /// </remarks>
        private List<ProcesoFacturaDetalle> ExtraerInformacionDetalle(List<ProcesoFacturaDetalle> listDetallesFactura)
        {
            var listDetalleFacturaRetorno = from
                            detalleFactura in listDetallesFactura
            ...
            return listDetalleFacturaRetorno.ToList();
        }
        ...
        private void GenerarEstadoCuenta() 
        {
            if (Page.IsValid)
            {
                ...
                string fechaActual = DateTime.Now.Year.ToString() + DateTime.Now.Month.ToString() + DateTime.Now.Day.ToString() + DateTime.Now.Minute.ToString() + DateTime.Now.Second.ToString() + DateTime.Now.Millisecond.ToString();

                VinculacionSeleccionada = BuscarVinculacionSeleccionada(Convert.ToInt32(identificadorEntidad.Text), indiceFila + 1);
                grillaEntidades.SelectedIndex = 0;
                ActivarCeldaGridView(grillaEntidades, indiceFila);
                ...
                if (ventasSeleccionadas.Count() > 0)
                {
                    ...
                    var listaConceptosCobro = WebService.Facturacion.ConsultarConceptos
                     (
                        new Atencion()
                        {
                            IdAtencion = Atencion.IdAtencion
                    ...
                    if (listaConceptosCobro.Ejecuto)
                    {
                        if (Atencion.Deposito != null)
                        {
                            Atencion.Deposito.Concepto = listaConceptosCobro.Objeto.ToList();
                        }
                    }
                    else
                    {
                        MostrarMensaje(listaConceptosCobro.Mensaje, TipoMensaje.Error);
                    }

                    Session["Atencion"] = Atencion;

                    System.Web.UI.ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "Reporte", String.Format("ecm = window.open(\"{0}?delay={1}\",'_blank');", Resources.AdministradorFacActividades.FacActividad_ReporteEstadoCuenta, fechaActual), true);
        ...
        private void ObtenerConceptosXDepositoFactura()
        {
            if (Atencion.Deposito != null)
            {
                ...
                {
                    var listaConceptos = (from fila in grillaEntidades.Rows.Cast<GridViewRow>()
        ...
        {
            var terceroResponsable = new Responsable()
            {
                CodigoEntidad = Settings.Default.General_CodigoEntidad,
            ...
            return terceroResponsable == null ? new Responsable() : terceroResponsable;
        }
        ...
        private List<int> ObtenerVentasMarcadas()
        {
            var ventasMarcadas = from
                                     ventas in grillaVentas.Rows.Cast<GridViewRow>()
                                 where
        ...
        private List<int> ObtenerVentasNoMarcadas()
        {
            ...
            return ventasNoMarcadas.ToList();
        }

        
        private List<Vinculacion> ObtenerVinculaciones()
        {
            List<Vinculacion> vinculaciones = new List<Vinculacion>();
            foreach (Vinculacion item in VinculacionesDeAtencion)
            {
                var listaVinculacionesSeleccionadas = (from fila in grillaEntidades.Rows.Cast<GridViewRow>()
                ...
                var listaObservaciones = (from fila in grillaEntidades.Rows.Cast<GridViewRow>()
                ...
                {
                    item.IndGenerar = 1;
                    vinculaciones.Add(item);
                }

                var listaVinculacionesNoSeleccionadas = (from fila in grillaEntidades.Rows.Cast<GridViewRow>()
                ...
                {
                    if (Convert.ToInt32((fila.FindControl(LABELIDCONTRATO) as Label).Text) == item.Contrato.Id
                        && Convert.ToInt32((fila.FindControl(LABELIDENTIDAD) as Label).Text) == item.Tercero.Id)
                    {
                        item.Observacion = (fila.FindControl(TXTOBSERVACION) as TextBox).Text;
                    }
                }
            }
        ...
        {
            var vinculaciones = (from item in grillaEntidades.Rows.Cast<GridViewRow>()
